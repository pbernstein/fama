...
#    print 'OpenOrdersWatcher %s : %s ' % (datetime.datetime.now(), msg.contract.m_symbol )
    print 'Canceling ORDER:  %s : %s %s %s %s %s %s' % (datetime.datetime.now(), msg.orderId, msg.contract.m_symbol, msg.order.m_action, msg.order.m_totalQuantity, msg.order.m_tif, msg.order.m_orderType)
    orderID = msg.orderId
