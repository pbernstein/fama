...
def pull_html(symbol, form,cik,date,temp_path,html_base_fn, html_extension_fn,link):
					...
					try:
						...
						if result == 0:
							try:
								...
								print "appending to retain: "+html_fn,new_html_fn
	...
	print "Attempting to load html content"
		...
		if new_file == 1:
			...
			if form[0:3] == "8-K":
				...
				try:
					...
					try:
						cursor.execute("insert into extract_history (`cik`,`name`,`period`,`form`,`date_id`,`html`,`html_link`,`status`) values (\""+str(cik)+"\",\""+name+"\", \""+period+"\", \""+str(form)+"\", "+str(date_id)+",\""+html_fn+"\",\""+htm_link+"\",\"S\")")
						conn.commit()
...
#
					...
					try:
						print "updateing insert extract history value with error"
						#print "insert into extract_history (`cik`,`name`,`period`,`form`,`date_id`,`html`,`html_link`) values ("+str(cik)+",\""+name+"\", \""+period+"\", \""+str(form)+"\", "+str(date_id)+",\"none\",\"none\")"
