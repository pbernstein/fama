...
def remove_bad_rows(table):
	...
	for entry in table:
		...
		try:
			buffer = entry[0]
			if buffer.strip()[0] == "(" and buffer.strip()[-1] == ")":
...
#
	...
	for entry in table:
		"""
		print
		print "date buffer "+str(date_buffer), str(len(date_buffer))
		print "year list "+str(year_list), str(len(year_list))
		...
		if pull_year == 1:
			...
			elif len(date_buffer) == 1 and len(year_list) == 3:
				#print date_buffer, str(len(date_buffer))
				#print year_list, str(len(year_list))
				db = date_buffer
				new_date_buffer = []
				new_db = []
				new_db.append([db[0][0][0], year_list[0], db[0][0][2],db[0][0][3]])
				new_date_buffer.append(new_db)

				new_db = []
				new_db.append([db[0][0][0], year_list[1], db[0][0][2],db[0][0][3]])
				new_date_buffer.append(new_db)

				new_db = []
				new_db.append([db[0][0][0], year_list[2], db[0][0][2],db[0][0][3]])
				new_date_buffer.append(new_db)
...
#['2011', '2010']
		...
		# If three, cut it down to two	
		temp = []	
		for i in [0,1]:
			try:
				temp.append(value_list[i])
			except:
				pass
...
def main(fn):
	...
	for row in table:
		...
		for entry in row:
			try:
				temp_row.append(str(entry)[:str(entry).index(":")].replace("\n",""))
			except:
